	...
	override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
		...
		categoryAdapter = CategoryAdapter(emptyList()) { category ->
			when (category.name) {
				...
				"Outdoor Activities" -> findNavController().navigate(R.id.action_main_to_farmsAndShit)
				"Dining Options" -> findNavController().navigate(R.id.action_main_to_cafesRestos)
				"Shopping Experiences" -> findNavController().navigate(R.id.action_main_to_othersNaks)
			}
		}
